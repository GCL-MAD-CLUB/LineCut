...
/**
 * In-memory mirror of the per-project state store persisted inside
 * WorkspaceConfig.xml, keyed by the project document id. It is loaded once at
 * startup and kept in sync whenever a project's state changes, so the project
 * open flow can read a project's recorded export settings synchronously.
 */
...
/**
 * Persists a project's export settings both locally and to the global store.
 * Passing `null` clears the stored entry so empty state never lingers.
 */
...
/**
 * Removes every per-project entry whose document id is not on the keep list.
 * The keep list is derived from the recently-opened projects list, so a project
 * is only forgotten once it also leaves that list.
 */
