...
- **全新导入浏览器**：按位置、面包屑和收藏夹浏览本地媒体；支持缩略图/列表视图、搜索、过滤、排序、隐藏文件和文件夹递归选择。
- **复制、校验与自动绑定**：导入时可复制媒体并进行 MD5 校验、创建新媒体箱，并按名称或媒体时间自动绑定音频和字幕。
- **分镜切点时间线**：在源监视器中显示、添加、选择、拖动、精确编辑、清除和恢复分镜切点；删除区间可见并在连续播放时自动跳过。
- **直接调整片段范围**：拖动时间线范围的入点、出点或中间手柄，修改字幕时间或分镜边界。
- **多分辨率缩略图**：按显示尺寸选择时间线缩略图，后台写入缓存并限制 FFmpeg 工作线程，改善长素材浏览时的响应。
...
完整变更、升级步骤和已知限制见 [0.3.2 新增功能](docs/user-guide/release-notes/v0.3.2.md)。
