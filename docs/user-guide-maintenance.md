...
- 信息架构参考 Adobe 桌面帮助中心：主页依次提供新增功能、开始使用、高频任务、常用文章和故障排查；侧边栏按用户从设置、导入、预览、整理到导出的任务顺序组织；
- 专题页使用“动词 + 对象”的任务标题。开头先说明能完成什么以及是否写入项目或磁盘，再依次给出“开始前”、编号步骤、结果/限制、注意事项和相关任务；没有前置条件时可省略“开始前”；
- 按稳定的功能主题组织页面；同一面板、同一工作区或同一完整流程的操作用二、三级标题集中在一页。仅当主题有独立入口、前置条件和可验证结果时再拆分页面；
- 每个可点击、可输入、可拖拽、可滚动、可用快捷键触发的用户操作，都必须在专题页中说明入口、前置条件、步骤、结果、禁用条件和高风险影响；
- 使用界面中的实际中文名称，行为不确定时先回到 `src` 和 `src-tauri` 核实；
- 更新说明按“用户获得什么能力—如何使用—升级影响—已知限制”组织；内部重构和依赖升级放在末尾，不用提交标题充当用户说明；
...
- 同时更新 `scripts/user-guide-operations.mjs` 的操作清单；`npm run docs:check` 会确认每项操作仍映射到存在的专题页和章节。
