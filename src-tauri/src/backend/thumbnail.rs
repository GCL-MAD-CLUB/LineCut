...
mod tests {
    ...
    fn candidate(time_us: i64, score: f64) -> CoverCandidate {
        CoverCandidate { time_us, score }
    }

    #[test]
    fn limits_long_videos_to_fixed_sample_budget() {
        let samples = analysis_sample_times(2 * 60 * 60 * 1_000_000);
        assert_eq!(samples.len(), MAX_ANALYSIS_SAMPLES);
        assert_eq!(samples[0], 0);
        assert!(samples.windows(2).all(|pair| pair[0] < pair[1]));
    }

    #[test]
    fn cache_layout_uses_hash_only_file_names_and_separate_folders() {
        let preferences = Preferences::default();
        let layout = thumbnail_cache_layout(&preferences, "fingerprint");
        let index_name = layout.index_path.file_stem().unwrap().to_string_lossy();
        let cache_name = layout.cache_path.file_stem().unwrap().to_string_lossy();
        assert_eq!(index_name.len(), 64);
        assert_eq!(cache_name.len(), 64);
        assert!(layout.index_path.ends_with(format!("{index_name}.mcdb")));
        assert!(layout.cache_path.ends_with(format!("{cache_name}.lctc")));
        assert!(layout
            .index_path
            .to_string_lossy()
            .contains(CACHE_INDEX_FOLDER));
        assert!(layout
            .cache_path
            .to_string_lossy()
            .contains(CACHE_FILES_FOLDER));
    }

    #[test]
    fn different_hash_shards_use_different_index_files() {
        let preferences = Preferences::default();
        let first = thumbnail_cache_layout(&preferences, "first");
        let mut second_fingerprint = "second".to_string();
        let second = loop {
            let layout = thumbnail_cache_layout(&preferences, &second_fingerprint);
            if layout.index_path != first.index_path {
                break layout;
            }
    ...
    #[test]
    fn selects_first_later_candidate_that_beats_prefix_best() {
        let mut candidates = vec![candidate(0, 0.1); 100];
        candidates[20] = candidate(20, 0.8);
        candidates[60] = candidate(60, 0.81);
        candidates[61] = candidate(61, 0.9);
        assert_eq!(select_cover_candidate(&candidates).time_us, 60);
    }

    #[test]
    fn falls_back_to_prefix_best_when_later_candidates_do_not_beat_it() {
        let mut candidates = vec![candidate(0, 0.1); 100];
        candidates[20] = candidate(20, 0.8);
        candidates[60] = candidate(60, 0.8);
        assert_eq!(select_cover_candidate(&candidates).time_us, 20);
    }

    #[test]
    fn information_score_rewards_detail_and_color_complexity() {
        let flat = vec![64_u8; ANALYSIS_FRAME_BYTES];
        let mut detailed = vec![0_u8; ANALYSIS_FRAME_BYTES];
        for y in 0..ANALYSIS_HEIGHT {
            for x in 0..ANALYSIS_WIDTH {
                let offset = (y * ANALYSIS_WIDTH + x) * 3;
                detailed[offset] = ((x * 29 + y * 11) % 256) as u8;
                detailed[offset + 1] = ((x * 7 + y * 31) % 256) as u8;
                detailed[offset + 2] = ((x * 19 + y * 5) % 256) as u8;
            }
        }
        assert!(frame_information_score(&detailed) > frame_information_score(&flat));
    }

    #[test]
    fn private_cache_round_trips_without_exposing_jpeg_header() {
        let jpeg = [0xff, 0xd8, 0xff, 0xe0, 1, 2, 3, 4];
        let transformed = transform_private_payload(&jpeg, "fingerprint", CACHE_KEY_CONTEXT);
        assert_ne!(&transformed[..4], &jpeg[..4]);
        assert_eq!(
            transform_private_payload(&transformed, "fingerprint", CACHE_KEY_CONTEXT),
            jpeg
        );
    }
}
